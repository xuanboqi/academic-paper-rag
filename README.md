...
- [本地使用说明](USE_CN.md)
- [英文文档](README_EN.md)
- [项目架构讲解 Notebook](project_architecture_cn.ipynb)
...
- `Agentic RAG`：基于 LangGraph 编排“任务识别 -> 查询改写 -> 证据检索 -> 答案生成”工作流，适合后续扩展文献综述、跨论文对比和图表公式检索。
- `多模态解析`：可本地接入 Qwen2.5-VL-3B-Instruct，将论文图表、公式和复杂页面解析成文本后向量化入库。
- 多模态脚本会按图表/公式/实验关键词对候选页打分，默认只解析每篇论文 Top 3 页面，避免本地视觉模型全量扫页过慢。
...
- Agentic RAG 页面会展示任务类型、改写后的检索查询和 LangGraph 执行步骤，方便观察多步推理过程。
- 多模态入库结果保存在 `literature_multimodal_chunks`，问答检索时可和普通正文 chunk 合并召回。
